...
void ordenarPorDni(){
    char linea[500];
    struct registros registro[500];
    int cantRegistros=0,x;
    FILE* apl;
    ...
    while(fgets(linea,sizeof(linea),apl)!=NULL){
        sscanf(linea,"%d , %s , %s", &registro[cantRegistros].dni, registro[cantRegistros].nombre, registro[cantRegistros].apellido);
        cantRegistros++;
    }
    
    for(x=0;x<cantRegistros;x++){
        printf("%d %s %s\n",registro[x].dni,registro[x].nombre,registro[x].apellido);
    }
